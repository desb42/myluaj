	...
	private VarInfo[][] findVariables() {
		...
		for ( int bi=0; bi<blocklist.length; bi++ ) {
			...
			for ( int pc=b0.pc0; pc<=b0.pc1; pc++ ) {
				...
				case Lua.OP_SETTABLE_b:
				case Lua.OP_SETTABLE_c:
				case Lua.OP_SETTABLE_d:
					a = Lua.GETARG_A( ins );
					b = Lua.GETARG_B( ins );
                                        if (op == Lua.OP_SETTABLE_a || op == Lua.OP_SETTABLE_c)
                                            b += 256;
					c = Lua.GETARG_C( ins );
                                        if (op == Lua.OP_SETTABLE_b || op == Lua.OP_SETTABLE_d)
                                            c += 256;
				...
				case Lua.OP_SETTABUP_a: /*	A B C	UpValue[A][RK(B)] := RK(C)			*/
				case Lua.OP_SETTABUP_b:
				case Lua.OP_SETTABUP_c:
				case Lua.OP_SETTABUP_d:
					b = Lua.GETARG_B( ins );
                                        if (op == Lua.OP_SETTABUP_a || op == Lua.OP_SETTABUP_c)
                                            b += 256;
					c = Lua.GETARG_C( ins );
                                        if (op == Lua.OP_SETTABUP_b || op == Lua.OP_SETTABUP_d)
                                            c += 256;
					if (!Lua.ISK(b)) v[b][pc].isreferenced = true;
					if (!Lua.ISK(c)) v[c][pc].isreferenced = true;
					break;
				...
				switch (Lua.GET_OPCODE(i)) {
					...
					case Lua.OP_SETTABUP_d: {
						int b = Lua.GETARG_B(i);
                                                ...
                                                if (op == Lua.OP_SETTABLE_a || op == Lua.OP_SETTABLE_c || op == Lua.OP_SETTABUP_a || op == Lua.OP_SETTABUP_c)
                                                    b += 256;
						if (Lua.ISK(b))
